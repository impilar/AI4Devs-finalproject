...
```bash
jq '.queue[] | select(.status == "backlog") | .sequence as $s | .id as $id | "\($s) \($id)"' 02-docs/02_3-engineering/implementation-queue-mvp.json | head -1
```

### Prompt recomendado

```
Implementa TASK-019 según 02-docs/02_3-engineering/implementation-queue-mvp.json
y 02-docs/02_1-product/user-stories/US-005.md (sección ### TASK-019).
...
2. Marcar misma task en `02-docs/02_1-product/user-stories/status-v1.json`
...
*Generado con el agente Implementation Planner a partir de `02-docs/02_2-architecture/lld/LLD-v1.md`, user stories enriquecidas MVP y `01-knowledge/templates/engineering/implementation-plan-template.md`.*
